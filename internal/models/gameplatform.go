...
type GamePlatform struct {
	...
	Files     []GamePlatformFile      `json:"files" yaml:"files"`         // 平台文件
	Installer []GamePlatformInstaller `json:"installer" yaml:"installer"` // 安装步骤
	CreatedAt time.Time               `json:"created_at" yaml:"created_at"`
	UpdatedAt time.Time               `json:"updated_at" yaml:"updated_at"`
}

// GamePlatformFile 平台文件
type GamePlatformFile struct {
	ID   string `json:"id" yaml:"id"`     // 文件ID
	Type string `json:"type" yaml:"type"` // 文件类型
	URL  string `json:"url" yaml:"url"`   // 文件URL
}

// GamePlatformInstaller 安装步骤
type GamePlatformInstaller struct {
